...
<p  style="color:white">" ---- "</p>
        <h3 style="color:rgb(13, 68, 252)"><b>แก้ไขรายละเอียด</b> <span class="badge badge-secondary">หมายเลขการร้องเรียนที่  {{$detail->issues_id}}</span> </h3>
<hr>
<form method="POST" action="{{url('/report/'.$detail->issues_id)}}" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    <div class="form-group row">
            ...
            <div class="col-sm-9">
            <textarea required class="form-control" name="detail" placeholder="รายละเอียดของปัญหา">{{$detail->getList->detail}}</textarea>
            </div>
      </div>
      ...
      <div class="form-group row">
            ...
            <div class="col-sm-9 text-center">
                    @if($detail->getList->picture == -1)
                    <img src="{{asset('img/noimage.jpg')}}" class="img-thumbnail" alt="Cinque Terre" width="304" height="236">
                    @else
                <img src="{{url($detail->getList->picture)}}" class="img-thumbnail" alt="Cinque Terre" width="304" height="236">
                    @endif
...
$(window).ready(function(){
        ...
        setTimeout(function() {
            document.getElementById("categorylist").value = "{{$detail->getList->category_id}}";
            fetch_data($('#categorylist').val());
                document.getElementById("building").value = "{{$detail->getList->building}}";
                document.getElementById("room").value = "{{$detail->getList->room}}";
                document.getElementById("campus").value = "{{$detail->getList->campus_id}}";
                setTimeout(function() {
                document.getElementById("subhere").value = "{{$detail->getList->sub_category_id}}";
            }, 1000);
        }, 500);
    });
</script>
